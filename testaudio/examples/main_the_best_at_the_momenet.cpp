...
// no aceptar paquetes demasiado adelantados
static const uint32_t MAX_AHEAD_WINDOW = 220;
// si la brecha entre highest y expected es muy grande, forzar rebuffer
                               ...
                               const uint8_t *payload)
{
    ...
    if (xSemaphoreTake(jitterMutex, portMAX_DELAY) == pdTRUE)
    {
        ...
        else
        {
            ...
            if (idx < 0)
            {
                // conservative behavior: keep current buffered window intact
                droppedPackets++;
                bufferFullDrops++;
                xSemaphoreGive(jitterMutex);
                return false;
...
static void statsTask(void *parameter)
{
    while (true)
    {
        ...
        Serial.printf(
            "[STATS] rx=%lu dec=%lu plc=%lu miss=%lu drop=%lu fullDrop=%lu aheadDrop=%lu dup=%lu late=%lu malformed=%lu decErr=%lu hardRebuf=%lu buffered=%d expected=%lu highest=%lu end=%d rebuf=%d finished=%d\n",
