...
volatile int16_t g_currentVolume = 0;
volatile bool g_volumeDirty = false;
...
{
    int16_t lastDrawnVolume = -1;

    while (true)
    {
        if (currentScreen == SCREEN_DEV)
        {
            int16_t volume = g_currentVolume;

            if (g_volumeDirty && abs(volume - lastDrawnVolume) > 300)
            {
...
void handleMainTouch(uint16_t x, uint16_t y)
{
    ...
    {
        drawButtonSafe(btnWifi, true);
        connectWiFi();
        vTaskDelay(250 / portTICK_PERIOD_MS);
        drawButtonSafe(btnWifi, false);
        return;
    }
    ...
    if (btnCommit.contains(x, y))
    {
        ...
        vTaskDelay(200 / portTICK_PERIOD_MS);
        drawButtonSafe(btnCommit, false);
        return;
    }

    if (btnCancel.contains(x, y))
    {
        drawButtonSafe(btnCancel, true);
        ...
        vTaskDelay(200 / portTICK_PERIOD_MS);
        drawButtonSafe(btnCancel, false);
        return;
    }
}